...
parser.add_argument('--normal_lookup_table')
parser.add_argument('--anomal_lookup_table')
...
def get_lookup_table_paths(args):
    normal_lookup_table_paths = str(args.normal_lookup_table).split(',')
    anomal_lookup_table_paths = str(args.anomal_lookup_table).split(',')
    return normal_lookup_table_paths, anomal_lookup_table_paths

# test_data = dset.ImageFolder('./one_class_test/' + classes[args.in_class])
transform = trn.Compose([trn.Resize(256),
                         trn.CenterCrop(224),
                         trn.ToTensor()])
# test_normal_lookup_tables_paths = ["/vol/ep/mm/anomaly_detection/data/rsna/local/dir_001/filtered-16-frame-middle-data-test-normal-7000-7099-clean/lookup_table.csv","/vol/ep/mm/anomaly_detection/data/rsna/local/dir_001/filtered-16-frame-middle-data-test-normal-7100-7199-clean/lookup_table.csv"]
test_normal_lookup_tables_paths, test_anomal_lookup_tables_paths = get_lookup_table_paths(args)
print(f"normal paths: {test_normal_lookup_tables_paths}")
print(f"anomal paths: {test_anomal_lookup_tables_paths}")

test_data_normal = RsnaDataset3dSliced(test_normal_lookup_tables_paths, transform)
...
test_data = PerturbDataset(test_data_normal)
...
if args.load != '':
    for i in range(1000 - 1, -1, -1):
        ...
        model_name = os.path.join(args.load, args.experiment_name + '_' + str(args.train_set) + '_' + str(i) + '.pt')
...
print('\nUsing train set', str(args.train_set), 'as typical data')
...
# test_anomal_lookup_tables_paths = ["/vol/ep/mm/anomaly_detection/data/rsna/local/dir_001/filtered-16-frame-middle-data-test-anomal-0-99-clean/lookup_table.csv","/vol/ep/mm/anomaly_detection/data/rsna/local/dir_001/filtered-16-frame-middle-data-test-anomal-100-199-clean/lookup_table.csv"]
test_data_anomal = RsnaDataset3dSliced(test_anomal_lookup_tables_paths, transform)
ood_data = PerturbDataset(test_data_anomal)
